...
const services = [
  ...
  },
  {
    ...
    description_ar: "أتمت مهامك المتكررة واحصل على عملاء محتملين مؤهلين تلقائيًا.",
...
async function seed() {
  console.log('Clearing existing services and addons...');
  
  // Clear services and addons
  await supabase.from('services').delete().neq('id', '00000000-0000-0000-0000-000000000000');
  await supabase.from('addons').delete().neq('id', '00000000-0000-0000-0000-000000000000');
  
  console.log('Inserting addons...');
  const { data: addonData, error: addonError } = await supabase.from('addons').insert(addons).select();
  if (addonError) {
    console.error('Failed to insert addons:', addonError);
    return;
  }
  console.log(`Successfully seeded ${addonData.length} addons.`);

  console.log('Inserting services...');
  const { data: svcData, error: svcError } = await supabase.from('services').insert(services).select();
  if (svcError) {
    console.error('Failed to insert services:', svcError);
    return;
  }
  console.log(`Successfully seeded ${svcData.length} services.`);
}
