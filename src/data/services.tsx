...
export function useServices(initialServices?: Service[]) {
  ...
  // FIX: was `> 1` which incorrectly re-fetched when exactly 1 service was passed from server
  const [loading, setLoading] = useState(initialServices && initialServices.length > 0 ? false : true);

  useEffect(() => {
    // FIX: was `> 1` — skip client-side fetch whenever server already provided data
